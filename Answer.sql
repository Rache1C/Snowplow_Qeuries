    ...
    SELECT rental.customer_id, rental.staff_id, AVG(payment.amount)
    FROM rental 
    JOIN payment
    ...
    BETWEEN '2005-01-01'
    AND '2005-12-31'
    GROUP BY rental.customer_id, rental.staff_id
    ORDER BY AVG(amount) DESC;
    
    /*3. Create a table, film_recommendations, which provides 10 film recommendations per customer. 
    ...
    /*4. Create a table, customer_lifecycle, with a primary key of customer_id. Please include all the required DDL. 
    ...
    /*5. Imagine that new rental data is being loaded into the database every hour. Assuming that the data is loaded sequentially, ordered by rental_date, 
